...
export type ActivityItem = {
  ...
  date: string
  amount: number
  // A credit-card payment: real on the statement, but your own money moving between two of your own
  ...
  return (
    <ul className="divide-y divide-line">
      {items.map((t) => {
        ...
        const internal = t.internalTransfer === true
        return (
          <li key={t.id} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="truncate text-xs text-muted">
                {internal ? 'Between your accounts' : t.category} · {shortDate(t.date)}
              </div>
            </div>
            <div
              className={`shrink-0 text-sm font-medium tabular-nums ${
                internal ? 'text-muted' : inflow ? 'text-emerald' : 'text-ink'
              }`}
            >
              {inflow && !internal ? '+' : ''}
              {money(display)}
